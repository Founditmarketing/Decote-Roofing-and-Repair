    ...
    <div className="bg-[#050505] min-h-screen text-white overflow-hidden py-32">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 }}}} className="max-w-4xl mx-auto">
            <motion.div variants={textRevealVariants} className="inline-flex items-center gap-3 mb-8">
              <Hammer className="w-5 h-5 text-[#3375C9]" />
              <span className="text-xs sm:text-sm font-bold tracking-[0.3em] uppercase text-[#3375C9]">Advanced Materials</span>
            </motion.div>
            <motion.h1 variants={textRevealVariants} className="text-5xl sm:text-7xl lg:text-8xl font-display font-bold tracking-tighter mb-8 leading-[0.9]">
              METAL <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-gray-600">ARCHITECTURE</span>
            </motion.h1>
            ...
            <motion.p variants={textRevealVariants} className="text-xl lg:text-2xl text-white/50 font-light max-w-2xl mx-auto leading-relaxed">
              The pinnacle of endurance. We specialize in high-gauge metal fortifications that deliver permanent aesthetic and thermal supremacy.
            </motion.p>
          </motion.div>
        </div>
        ...
        >
          <div className="relative h-[400px] md:h-[600px] w-full rounded-2xl overflow-hidden glass-dark border border-white/10 p-2 shadow-2xl group">
            <div className="w-full h-full rounded-xl overflow-hidden relative">
              <img
                src="/Metal-Roofing-1.png"
                alt="Metal Architecture Form"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-32">
            <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}>
              <span className="text-[#3375C9] text-xs font-bold uppercase tracking-[0.4em] mb-4 block">Structural Profiles</span>
              <h2 className="text-4xl sm:text-5xl font-display font-bold tracking-tighter text-white/90 mb-6">SEAMLESS INTEGRATION</h2>
              <div className="w-16 h-px bg-[#004AAC] mb-8"></div>
              <div className="space-y-6 text-white/50 font-light leading-relaxed">
                <p>Metal systems eliminate standard failure points. Whether adopting a standing seam profile or heavy-duty corrugated architecture, the result is an impenetrable shield against terminal velocity impacts.</p>
                <div className="glass p-6 rounded-xl border border-white/5 space-y-4 mt-8">
                  <div className="flex items-center gap-4"><CheckCircle2 className="w-5 h-5 text-[#3375C9]"/><span className="text-white/80">Standing Seam Engineering</span></div>
                  <div className="flex items-center gap-4"><CheckCircle2 className="w-5 h-5 text-[#3375C9]"/><span className="text-white/80">Hyper-Reflective Thermal Coatings</span></div>
                  <div className="flex items-center gap-4"><CheckCircle2 className="w-5 h-5 text-[#3375C9]"/><span className="text-white/80">Class 4 Impact Resistance</span></div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1], delay: 0.2 }} className="relative h-[500px]">
              ...
              <div className="h-full w-full glass-dark border border-white/10 rounded-2xl p-2 overflow-hidden shadow-2xl">
                <img src="/modern-standing-seam-roof.png" alt="Metal Seam Profiling" className="w-full h-full object-cover rounded-xl filter grayscale contrast-125 hover:grayscale-0 transition-all duration-700"/>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-24 bg-[#0a0a0a] border-t border-white/5 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <h2 className="text-3xl md:text-5xl font-display font-bold tracking-tighter text-white mb-6">COMMAND ENDURANCE</h2>
          <p className="text-lg text-white/50 font-light mb-10 max-w-xl mx-auto">Establish a permanent architectural defense. Request a specialized metal framework analysis.</p>
          <NavLink to="/contact" className="group relative inline-flex items-center justify-center px-10 py-5 bg-white text-black font-bold uppercase tracking-widest text-sm rounded-xl overflow-hidden hover:text-white transition-colors duration-300">
            ...
            <span className="relative z-10 flex items-center gap-3">Authorize Consultation <ArrowRight className="w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300" /></span>
          </NavLink>
        </div>
      </section>
    </div>
