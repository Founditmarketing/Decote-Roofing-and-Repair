...
export default function SidingGuttersPage() {
  ...
  return (
    <div className="bg-[#050505] min-h-screen text-white overflow-hidden py-32">
      <PageSEO
        title="Siding & Gutters | Ducote Roofing & Repair — Alexandria, LA"
        description="Complete exterior protection with expert siding installation and gutter systems engineered for superior water management in Alexandria, LA."
        path="/siding-gutters"
        schema={buildServiceSchema('Siding & Gutter Installation', '/siding-gutters')}
      />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 }}}} className="max-w-4xl mx-auto">
            <motion.div variants={textRevealVariants} className="inline-flex items-center gap-3 mb-8">
              <Droplets className="w-5 h-5 text-[#3375C9]" />
              <span className="text-xs sm:text-sm font-bold tracking-[0.3em] uppercase text-[#3375C9]">Exterior Defense</span>
            </motion.div>
            <motion.h1 variants={textRevealVariants} className="text-5xl sm:text-7xl lg:text-8xl font-display font-bold tracking-tighter mb-8 leading-[0.9]">
              SIDING & <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-gray-600">WATER MANAGEMENT</span>
            </motion.h1>
            ...
            <motion.p variants={textRevealVariants} className="text-xl lg:text-2xl text-white/50 font-light max-w-2xl mx-auto leading-relaxed">
              Complete perimeter fortification. Synthesizing advanced cladding systems with algorithmic water displacement engineering.
            </motion.p>
          </motion.div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-32">
            <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}>
              <span className="text-[#3375C9] text-xs font-bold uppercase tracking-[0.4em] mb-4 block">Perimeter Security</span>
              <h2 className="text-4xl sm:text-5xl font-display font-bold tracking-tighter text-white/90 mb-6">CLADDING & GUTTERS</h2>
              <div className="w-16 h-px bg-[#004AAC] mb-8"></div>
              <div className="space-y-6 text-white/50 font-light leading-relaxed">
                <p>A compromised facade risks total systemic failure. We deploy high-impact composite and steel cladding, paired with seamless extrusion gutters, effectively nullifying hydrostatic pressure on the exterior foundation.</p>
                <div className="glass p-6 rounded-xl border border-white/5 space-y-4 mt-8">
                  <div className="flex items-center gap-4"><CheckCircle2 className="w-5 h-5 text-[#3375C9]"/><span className="text-white/80">Seamless Aluminum K-Style Profiling</span></div>
                  <div className="flex items-center gap-4"><CheckCircle2 className="w-5 h-5 text-[#3375C9]"/><span className="text-white/80">Hardie Fiber Cement Formations</span></div>
                  <div className="flex items-center gap-4"><CheckCircle2 className="w-5 h-5 text-[#3375C9]"/><span className="text-white/80">Custom Nickel Gap Shiplap Exteriors</span></div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1], delay: 0.2 }} className="relative h-[500px]">
              ...
              <div className="h-full w-full glass-dark border border-white/10 rounded-2xl p-2 overflow-hidden shadow-2xl">
                <img src="/Siding-Gutters-1.png" alt="Seamless Extrusions" className="w-full h-full object-cover rounded-xl filter grayscale contrast-125 hover:grayscale-0 transition-all duration-700"/>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-24 bg-[#0a0a0a] border-t border-white/5 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <h2 className="text-3xl md:text-5xl font-display font-bold tracking-tighter text-white mb-6">ENGAGE WATER DEFENSE</h2>
          <p className="text-lg text-white/50 font-light mb-10 max-w-xl mx-auto">Authorize an objective exterior facade assessment and secure your foundation today.</p>
          <NavLink to="/contact" className="group relative inline-flex items-center justify-center px-10 py-5 bg-white text-black font-bold uppercase tracking-widest text-sm rounded-xl overflow-hidden hover:text-white transition-colors duration-300">
            ...
            <span className="relative z-10 flex items-center gap-3">Deploy Technical Squad <ArrowRight className="w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300" /></span>
          </NavLink>
        </div>
      </section>
    </div>
